// Wipes every DB row that references any Bridgify-related slug so you can
// re-onboard cleanly. Does NOT touch .env or config/vendors/*.md files.
//
// Run: node scripts/reset_bridgify.js

import 'dotenv/config';
import { query, closePool } from '../src/db/client.js';

const SLUGS = ['bridgify', 'bridgify-attraction-api', 'bridgify-attractions', 'bridgify-experiences'];

const log = (event, extra = {}) =>
  console.log(JSON.stringify({ event, ...extra }));

const wipe = async (table, column = 'supplier_slug', extraSql = '') => {
  const r = await query(
    `DELETE FROM ${table} WHERE ${column} = ANY($1::text[]) ${extraSql}`,
    [SLUGS]
  );
...
const main = async () => {
  ...
  // Verify nothing Bridgify-related remains
  for (const t of [
